...
${adPack.captions.map((c, i) => `**${i + 1}. [${c.type}]**\n${c.text}\n\n${c.hashtags.join(' ')}\n`).join('\n')}
  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col gap-8 relative">
      ...
      {copied && (
        <div className="fixed top-20 right-6 z-50 bg-blue-600 text-white font-mono text-xs px-4 py-3 rounded-xl shadow-xl border border-blue-400/30 flex items-center gap-2 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-950/60 p-6 border border-slate-800/80 rounded-2xl mb-2 relative backdrop-blur-sm shadow-xl">
         ...
         <div className="flex flex-wrap gap-2.5 sm:gap-3 items-center">
            ...
            <Link 
              to="/" 
              className="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white text-[10px] font-bold tracking-widest uppercase hover:opacity-90 transition-opacity flex items-center justify-center gap-1 rounded-xl shadow-md border border-blue-500/40"
            >
        ...
        <div className="flex border-b border-slate-800/60 overflow-x-auto bg-slate-900/40 p-2.5 gap-2">
            {['hooks', 'script', 'ctas', 'captions'].map(tab => (
                <button
                    ...
                    className={`px-5 py-2.5 text-[10px] font-bold uppercase tracking-widest whitespace-nowrap transition-all rounded-xl cursor-pointer ${activeTab === tab ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/10 border border-blue-550/45' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/30'}`}
                >
                    {tab}
                </button>
            ))}
                            ...
                            <div key={i} className="p-6 border border-slate-800/85 hover:border-slate-700 bg-slate-900/15 hover:bg-slate-900/25 relative overflow-hidden rounded-2xl shadow-md transition-all group flex flex-col md:flex-row md:items-start gap-4">
                                <div className="flex-1">
                                    <div className="flex justify-between items-center mb-4">
                                        <span className="inline-block px-3 py-0.5 bg-blue-500/5 border border-blue-500/10 text-[9px] text-blue-400 font-mono uppercase tracking-wider rounded-lg font-bold font-bold">
